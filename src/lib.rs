...
impl NodeId {
    ...
    #[inline(always)] pub fn is_valid(self) -> bool { self.0 != INVALID }
...
#[derive(Clone, Debug)]
struct HalfEdge { target: NodeId, edge_id: EdgeId, next: u32 }

#[derive(Clone, Debug)]
pub struct Edge { pub src: NodeId, pub dst: NodeId }
...
impl Graph {
    ...
    #[inline] pub fn num_nodes(&self) -> usize { self.heads.len() }
    ...
    pub fn add_edge(&mut self, u: NodeId, v: NodeId) -> EdgeId {
        ...
        let idx_vu = idx_uv + 1;
        self.half_edges.push(HalfEdge { target: v, edge_id: eid, next: self.heads[u.idx()] });
        self.heads[u.idx()] = idx_uv;
        self.half_edges.push(HalfEdge { target: u, edge_id: eid, next: self.heads[v.idx()] });
        self.heads[v.idx()] = idx_vu;
        eid
    }
    #[inline] pub fn edge(&self, eid: EdgeId) -> &Edge { &self.edges[eid.idx()] }
    pub fn neighbors(&self, u: NodeId) -> NeighborIter<'_> {
        NeighborIter { graph: self, current: self.heads[u.idx()] }
    }
    pub fn degree(&self, u: NodeId) -> usize { self.neighbors(u).count() }
...
pub struct NeighborIter<'a> { graph: &'a Graph, current: u32 }
impl<'a> Iterator for NeighborIter<'a> {
    type Item = (NodeId, EdgeId);
    #[inline]
    fn next(&mut self) -> Option<Self::Item> {
        if self.current == INVALID { return None; }
...
pub enum SpqrNodeType { S, P, R }

#[derive(Clone, Debug)]
pub struct SkeletonEdge {
    pub src: NodeId, pub dst: NodeId,
    pub real_edge: EdgeId, pub virtual_id: u32,
    pub twin_tree_node: TreeNodeId, pub twin_edge_idx: u32,
}
...
impl Skeleton {
    fn new() -> Self { Skeleton { num_nodes: 0, edges: Vec::new(), node_to_original: Vec::new() } }
    pub fn poles(&self) -> (NodeId, NodeId) { (self.node_to_original[0], self.node_to_original[1]) }
    pub fn num_edges(&self) -> usize { self.edges.len() }
    pub fn real_edges(&self) -> impl Iterator<Item = (usize, &SkeletonEdge)> {
        self.edges.iter().enumerate().filter(|(_, e)| e.real_edge.is_valid())
    }
    pub fn virtual_edges(&self) -> impl Iterator<Item = (usize, &SkeletonEdge)> {
        self.edges.iter().enumerate().filter(|(_, e)| e.twin_tree_node.is_valid())
    }
}
...
impl SpqrTree {
    ...
    #[inline] pub fn node(&self, id: TreeNodeId) -> &SpqrTreeNode { &self.nodes[id.idx()] }
    pub fn tree_node_of_edge(&self, eid: EdgeId) -> TreeNodeId { self.edge_to_tree_node[eid.idx()] }
    pub fn count_by_type(&self) -> (usize, usize, usize) {
        let (mut s, mut p, mut r) = (0, 0, 0);
        for n in &self.nodes { match n.node_type {
            SpqrNodeType::S => s += 1, SpqrNodeType::P => p += 1, SpqrNodeType::R => r += 1,
        }}
        (s, p, r)
    }
    pub fn iter(&self) -> impl Iterator<Item = (TreeNodeId, &SpqrTreeNode)> {
        self.nodes.iter().enumerate().map(|(i, n)| (TreeNodeId(i as u32), n))
    }
}
...
struct StackEdge { src: u32, dst: u32, eid: u32 }

#[derive(Clone, Debug)]
struct SplitComponent { edges: Vec<StackEdge>, pole_a: u32, pole_b: u32 }
...
fn build_spqr_tree_filtered(graph: &Graph, is_self_loop: &[bool]) -> SpqrTree {
    ...
    if m_real == 1 {
        let mut eid_real = 0;
        for i in 0..m { if !is_self_loop[i] { eid_real = i; break; } }
        let e = graph.edge(EdgeId(eid_real as u32));
        let mut sk = Skeleton::new();
        sk.num_nodes = 2;
        sk.node_to_original = vec![e.src, e.dst];
        sk.edges.push(SkeletonEdge {
            src: NodeId(0), dst: NodeId(1), real_edge: EdgeId(eid_real as u32),
            virtual_id: INVALID, twin_tree_node: TreeNodeId::INVALID, twin_edge_idx: INVALID,
        });
        let mut ettn = vec![TreeNodeId::INVALID; m];
        ettn[eid_real] = TreeNodeId(0);
        return SpqrTree {
            nodes: vec![SpqrTreeNode { node_type: SpqrNodeType::P, skeleton: sk, parent: TreeNodeId::INVALID, children: Vec::new() }],
            root: TreeNodeId(0), edge_to_tree_node: ettn,
        };
    }
    ...
    let (multi_comps, synthetic, consumed) = split_multi_edges(graph, &mut next_virtual, is_self_loop);
    ...
    let ref_weid = {
        ...
        for i in 0..wg_m {
            ...
            if e.src != e.dst { found = EdgeId(i as u32); break; }
        }
        found
    };
    ...
    let mut wcomps = if used_triconn {
        ...
        let edges: Vec<StackEdge> = (0..wg_m)
            .map(|i| { let e = wg.edge(EdgeId(i as u32)); StackEdge { src: e.src.0, dst: e.dst.0, eid: i as u32 } })
            .collect();
        if edges.is_empty() { Vec::new() }
        else { let (pa,pb) = (edges[0].src, edges[0].dst); vec![SplitComponent { edges, pole_a: pa, pole_b: pb }] }
    };

    for comp in &mut wcomps {
        for se in &mut comp.edges {
            let i = se.eid as usize;
            if i < weid_to_label.len() { se.eid = weid_to_label[i]; }
        }
    }
...
fn build_parallel_case(graph: &Graph, is_self_loop: &[bool]) -> SpqrTree {
    ...
    for i in 0..m {
        ...
        let e = graph.edge(EdgeId(i as u32));
        sk.edges.push(SkeletonEdge {
            src: if e.src == NodeId(0) { NodeId(0) } else { NodeId(1) },
            dst: if e.src == NodeId(0) { NodeId(1) } else { NodeId(0) },
            real_edge: EdgeId(i as u32), virtual_id: INVALID,
            twin_tree_node: TreeNodeId::INVALID, twin_edge_idx: INVALID,
        });
        ettn[i] = TreeNodeId(0);
    }
    SpqrTree {
...
fn split_multi_edges(
    graph: &Graph, next_virtual: &mut u32, is_self_loop: &[bool],
    ...
    for i in 0..m {
        if is_self_loop[i] { continue; }
        let e = graph.edge(EdgeId(i as u32));
        let (a, b) = if e.src.0 <= e.dst.0 { (e.src.0, e.dst.0) } else { (e.dst.0, e.src.0) };
    ...
    for (&(a, b), eids) in &pair_to_eids {
        if eids.len() >= 2 {
            let vid = *next_virtual; *next_virtual += 1;
            let mut edges: Vec<StackEdge> = eids.iter()
                .map(|&eid| { let e = graph.edge(EdgeId(eid)); StackEdge { src: e.src.0, dst: e.dst.0, eid } }).collect();
            edges.push(StackEdge { src: a, dst: b, eid: vid });
            p_comps.push(SplitComponent { edges, pole_a: a, pole_b: b });
            for &eid in eids { consumed[eid as usize] = true; }
...
fn triconn_decompose(
    ...
    macro_rules! new_edge {
        ($src:expr, $dst:expr, $orig:expr, $et:expr) => {{
            ...
            me_src.push($src); me_dst.push($dst); me_orig.push($orig);
            me_etype.push($et); me_start.push(false);
            me_adj_v.push(INVALID); me_adj_p.push(INVALID);
    ...
    struct HpEntry { val: i32, next: u32, deleted: bool }
    ...
    for v in 0..n { degree[v] = graph.degree(NodeId(v as u32)) as i32; }
    ...
    let mut etype_orig = vec![0u8; m];
    {
        ...
        for i in 0..consumed.len().min(m) {
            if consumed[i] { seen[i] = true; etype_orig[i] = 3; }
        }
        let s0 = 0u32;
        nc += 1; number[s0 as usize] = nc;
        lp1[s0 as usize] = nc; lp2[s0 as usize] = nc;
        struct F { v: u32, he: u32 }
        let mut stk = vec![F { v: s0, he: graph.heads[s0 as usize] }];
        while let Some(fr) = stk.last_mut() {
            ...
            if fr.he == INVALID {
                ...
                if let Some(p) = stk.last() {
                    ...
                    if a < lp1[pv] { lp2[pv] = std::cmp::min(lp1[pv], b); lp1[pv] = a; }
                    else if a == lp1[pv] { lp2[pv] = std::cmp::min(lp2[pv], b); }
                    else { lp2[pv] = std::cmp::min(lp2[pv], a); }
                }
                continue;
            }
            let he = &graph.half_edges[fr.he as usize];
            fr.he = he.next;
            let w = he.target.0; let ei = he.edge_id.0 as usize;
            if seen[ei] { continue; } seen[ei] = true;
            if number[w as usize] == 0 {
                etype_orig[ei] = 1; nc += 1; number[w as usize] = nc;
                father[w as usize] = v as i32; lp1[w as usize] = nc; lp2[w as usize] = nc;
                tree_arc[w as usize] = ei as u32;
                stk.push(F { v: w, he: graph.heads[w as usize] });
            } else {
                etype_orig[ei] = 2;
                let nw = number[w as usize];
                if nw < lp1[v as usize] { lp2[v as usize] = lp1[v as usize]; lp1[v as usize] = nw; }
                else if nw > lp1[v as usize] { lp2[v as usize] = std::cmp::min(lp2[v as usize], nw); }
            }
        }
    ...
    for i in 0..m {
        if etype_orig[i] == 0 || etype_orig[i] == 3 { continue; }
        let w = edst[i]; let vs = esrc[i];
        let phi = if etype_orig[i] == 2 { 3 * number[w as usize] + 1 }
                  else if lp2[w as usize] < number[vs as usize] { 3 * lp1[w as usize] }
                  else { 3 * lp1[w as usize] + 2 };
        if phi >= 1 && phi <= maxb { bkt[phi as usize].push(i as u32); }
    }
    let mut oadj: Vec<Vec<u32>> = vec![Vec::new(); n];
    for phi in 1..=(maxb as usize) {
        for &ei in &bkt[phi] { oadj[esrc[ei as usize] as usize].push(ei); }
    }
    ...
    let mut hp_init: Vec<Vec<(i32, u32)>> = vec![Vec::new(); n];
    {
        ...
        struct PF { v: u32, idx: usize, pend: bool }
        let mut pfs = vec![PF { v: s0, idx: 0, pend: false }];
        while let Some(fr) = pfs.last_mut() {
            if fr.pend { fr.pend = false; nc -= 1; }
            let v = fr.v;
            if fr.idx >= oadj[v as usize].len() { pfs.pop(); continue; }
            let ei = oadj[v as usize][fr.idx] as usize;
            fr.idx += 1;
            let w = edst[ei];
            if np { np = false; startf[ei] = true; }
            if etype_orig[ei] == 1 {
                fr.pend = true;
                newnum[w as usize] = nc - nd_arr[w as usize] + 1;
                pfs.push(PF { v: w, idx: 0, pend: false });
            } else {
    ...
    for v in 0..n { o2n[number[v] as usize] = newnum[v]; }
    for v in 0..n { lp1[v] = o2n[lp1[v] as usize]; lp2[v] = o2n[lp2[v] as usize]; }
    for v in 0..n { nodeat[newnum[v] as usize] = v as u32; }
    ...
    for v in 0..n {
        for &(val, eidx) in &hp_init[v] {
            ...
            hp_arena.push(HpEntry { val, next: INVALID, deleted: false });
    ...
    macro_rules! high {
        ($v:expr) => {{
            ...
            if hp_head[__vi] == INVALID { 0i32 } else { hp_arena[hp_head[__vi] as usize].val }
        }}
    }
    ...
    macro_rules! adj_front {
        ($v:expr) => {{
            ...
            if h == INVALID { None } else { Some((al_edge[h as usize], h)) }
        }}
    }
    macro_rules! adj_count {
        ($v:expr) => { ah_count[$v as usize] }
    }
    ...
    macro_rules! del_adj {
        ($ei:expr) => {
            ...
            if __v != INVALID as usize {
                ...
                if __prev != INVALID { al_next[__prev as usize] = __next; }
                else { ah_head[__v] = __next; }
                if __next != INVALID { al_prev[__next as usize] = __prev; }
                else { ah_tail[__v] = __prev; }
                ah_count[__v] -= 1;
            }
        }
    }
        ...
        ($v:expr, $slot:expr) => {
            {
                ...
                if __prev != INVALID { al_next[__prev as usize] = __next; }
                else { ah_head[__v2] = __next; }
                if __next != INVALID { al_prev[__next as usize] = __prev; }
                else { ah_tail[__v2] = __prev; }
                ah_count[__v2] -= 1;
            }
        }
    }
    ...
    macro_rules! se {
        ($ei:expr) => {
            StackEdge { src: me_src[$ei as usize], dst: me_dst[$ei as usize], eid: me_orig[$ei as usize] }
        }
    }
    ...
    struct PS { v:u32, vn:i32, outv:i32, cur:u32, after:bool, ei:u32, w:u32, wn:i32, it:u32, tei:u32 }

    let s0 = 0u32;
    let (fei, fpos) = adj_front!(s0).expect("start vertex has no adj");
    let fw = me_dst[fei as usize];
    let mut cs: Vec<PS> = vec![PS {
        v:s0, vn:newnum[s0 as usize], outv: adj_count!(s0),
        cur:fpos, after:false, ei:fei, w:fw, wn:newnum[fw as usize], it:fpos, tei:INVALID,
    }];
    ...
    while !cs.is_empty() {
        ...
        if !cs[idx].after && me_etype[cs[idx].ei as usize] == 1 {
            ...
            if me_start[ei as usize] {
                if ta[top] > lp1[w as usize] {
                    let mut y = 0i32; let mut bv;
                    loop {
                        y = std::cmp::max(y, th[top]); bv = tb[top]; top -= 1;
                        if ta[top] <= lp1[w as usize] { break; }
                    }
                    top += 1; th[top] = y; ta[top] = lp1[w as usize]; tb[top] = bv;
                } else {
                    top += 1;
                    th[top] = newnum[w as usize] + nd_arr[w as usize] - 1;
                    ta[top] = lp1[w as usize]; tb[top] = vn;
                }
                top += 1; ta[top] = -1;
            }
            cs[idx].after = true;
            ...
            if let Some((ce, cp)) = adj_front!(w) {
                ...
                cs.push(PS {
                    v:w, vn:newnum[w as usize], outv:adj_count!(w),
                    cur:cp, after:false, ei:ce, w:cw, wn:newnum[cw as usize], it:cp, tei:INVALID,
                });
            }
            continue;
            ...
            while vn != 1 && (
                ta[top] == vn ||
                (degree[w as usize] == 2 && adj_front!(w).map_or(false, |(fe,_)| newnum[me_dst[fe as usize] as usize] > wn))
            ) {
                let a = ta[top]; let b = tb[top];
                if a == vn && father[nodeat[b as usize] as usize] == nodeat[a as usize] as i32 {
                    top -= 1;
                } else {
                    let mut eab: Option<u32> = None;

                    if degree[w as usize] == 2 && adj_front!(w).map_or(false, |(fe,_)| newnum[me_dst[fe as usize] as usize] > wn) {
                        let e1 = estack.pop().unwrap();
                        let e2 = estack.pop().unwrap();
                        del_adj!(e2);
                        let x = me_dst[e2 as usize];
                        degree[x as usize] -= 1; degree[v as usize] -= 1;
                        let vid = *next_virtual; *next_virtual += 1;
                        let ev = new_edge!(v, x, vid, 1);
                        comps.push(SplitComponent {
                            edges: vec![se!(e1), se!(e2), StackEdge{src:v,dst:x,eid:vid}],
                            pole_a: v, pole_b: x,
                        });
                        if let Some(&et) = estack.last() {
                            if me_src[et as usize] == x && me_dst[et as usize] == v {
                                let eab2 = estack.pop().unwrap();
                                del_adj!(eab2); del_high!(eab2);
                                eab = Some(eab2);
                            }
                        }
                        let mut cur_virt = ev; let cur_vid = vid;
                        if let Some(eab_v) = eab {
                            let vid2 = *next_virtual; *next_virtual += 1;
                            let nv2 = new_edge!(v, x, vid2, 1);
                            comps.push(SplitComponent {
                                edges: vec![se!(eab_v), StackEdge{src:v,dst:x,eid:cur_vid}, StackEdge{src:v,dst:x,eid:vid2}],
                                pole_a: v, pole_b: x,
                            });
                            degree[x as usize] -= 1; degree[v as usize] -= 1;
                            cur_virt = nv2;
                        ...
                        degree[x as usize] += 1; degree[v as usize] += 1;
                        father[x as usize] = v as i32; tree_arc[x as usize] = cur_virt;
                        me_etype[cur_virt as usize] = 1;
                        w = x; wn = newnum[w as usize];
                    } else {
                        let h = th[top]; top -= 1;
                        ...
                        let pa = nodeat[a as usize]; let pb = nodeat[b as usize];
                        let vid = *next_virtual; *next_virtual += 1;
                        let ev = new_edge!(pa, pb, vid, 1);
                        ce.push(StackEdge{src:pa,dst:pb,eid:vid});
                        comps.push(SplitComponent { edges: ce, pole_a: pa, pole_b: pb });
                        let x = pb;
                        let mut cur_virt = ev; let cur_vid = vid;
                        if let Some(eab_v) = eab {
                            let vid2 = *next_virtual; *next_virtual += 1;
            ...
            if lp2[w as usize] >= vn && lp1[w as usize] < vn
                && (father[v as usize] != s0 as i32 || cs[idx].outv >= 2)
            {
                let l1 = lp1[w as usize];
                let mut ce: Vec<StackEdge> = Vec::new();
                let mut xx = 0i32; let mut yy = 0i32;
                while let Some(&et) = estack.last() {
                    xx = newnum[me_src[et as usize] as usize];
                    yy = newnum[me_dst[et as usize] as usize];
                    if !((wn<=xx && xx<wn+nd_arr[w as usize]) || (wn<=yy && yy<wn+nd_arr[w as usize])) { break; }
                    let eh = estack.pop().unwrap();
                    del_high!(eh);
                    ce.push(se!(eh));
                ...
                let cur_vid = vid;
                ce.push(StackEdge{src:v,dst:pl,eid:vid});
                comps.push(SplitComponent { edges: ce, pole_a: v, pole_b: pl });

                if (xx==vn && yy==l1) || (yy==vn && xx==l1) {
                    if let Some(eh) = estack.pop() {
                        if !(me_adj_v[eh as usize]==v && me_adj_p[eh as usize]==itp) { del_adj!(eh); }
                        let vid2 = *next_virtual; *next_virtual += 1;
                        let nv2 = new_edge!(v, pl, vid2, 1);
                        comps.push(SplitComponent {
                            edges: vec![se!(eh), StackEdge{src:v,dst:pl,eid:cur_vid}, StackEdge{src:v,dst:pl,eid:vid2}],
                            pole_a: v, pole_b: pl,
                        });
                        me_hi_slot[nv2 as usize] = me_hi_slot[eh as usize];
                        degree[v as usize] -= 1; degree[pl as usize] -= 1;
                        ev = nv2;
                ...
                if pl as i32 != father[v as usize] {
                    ...
                    if me_hi_slot[ev as usize] == INVALID && high!(pl) < vn {
                        let slot = hp_arena.len() as u32;
                        hp_arena.push(HpEntry { val: vn, next: hp_head[pl as usize], deleted: false });
                        hp_head[pl as usize] = slot;
                        if hp_tail[pl as usize] == INVALID { hp_tail[pl as usize] = slot; }
                    ...
                    comps.push(SplitComponent {
                        edges: vec![StackEdge{src:v,dst:pl,eid:cur_vid}, StackEdge{src:pl,dst:v,eid:vid2}, se!(tav)],
                        pole_a: pl, pole_b: v,
                    });
                    tree_arc[v as usize] = nv2; me_etype[nv2 as usize] = 1;
            ...
            if me_start[ei as usize] {
                if ta[top] > wn {
                    ...
                    top += 1; th[top] = y; ta[top] = wn; tb[top] = bv;
                } else {
                    top += 1; th[top] = vn; ta[top] = wn; tb[top] = vn;
                }
            }
            estack.push(ei);
        }
        ...
        if let Some((np, ne, nw, nwn)) = next_slot!(cs[idx].cur) {
            cs[idx].cur = np; cs[idx].ei = ne; cs[idx].w = nw; cs[idx].wn = nwn;
        } else {
            cs.pop();
        }
    }
    ...
    if !estack.is_empty() {
        ...
        while let Some(ei) = estack.pop() { rem.push(se!(ei)); }
        let (pa, pb) = (rem[0].src, rem[0].dst);
        comps.push(SplitComponent { edges: rem, pole_a: pa, pole_b: pb });
    }
...
fn combine_components(multi: Vec<SplitComponent>, work: Vec<SplitComponent>, next_virtual: &mut u32) -> Vec<SplitComponent> {
    let mut out = multi;
...
fn split_internal_parallels(comp: SplitComponent, next_virtual: &mut u32) -> Vec<SplitComponent> {
    ...
    for e in &comp.edges { verts.insert(e.src, ()); verts.insert(e.dst, ()); }
    if verts.len() <= 2 { return vec![comp]; }
    ...
    if !groups.values().any(|g| g.len() >= 2) { return vec![comp]; }
    ...
    for (&(a, b), es) in groups.iter() {
        if es.len() >= 2 {
            ...
            bond_edges.push(StackEdge { src: a, dst: b, eid: vid });
            result.push(SplitComponent { edges: bond_edges, pole_a: a, pole_b: b });
            remainder_edges.push(StackEdge { src: a, dst: b, eid: vid });
        } else { remainder_edges.push(es[0]); }
    }
    result.push(SplitComponent { edges: remainder_edges, pole_a: comp.pole_a, pole_b: comp.pole_b });
    result
}

fn classify_component(comp: &SplitComponent) -> SpqrNodeType {
    let mut deg: HashMap<u32, u32> = HashMap::new();
    for e in &comp.edges { *deg.entry(e.src).or_default() += 1; *deg.entry(e.dst).or_default() += 1; }
    let v = deg.len(); let e = comp.edges.len();
    if v == 2 && e >= 2 { return SpqrNodeType::P; }
    if e == v && e >= 3 && deg.values().all(|&d| d == 2) { return SpqrNodeType::S; }
    SpqrNodeType::R
}

fn merge_same_type_components(comps: &mut Vec<SplitComponent>, m: usize) {
    let ctype: Vec<SpqrNodeType> = comps.iter().map(|c| classify_component(c)).collect();
    ...
    for (ci, comp) in comps.iter().enumerate() {
        for e in &comp.edges {
            if (e.eid as usize) >= m {
                if !comp1.contains_key(&e.eid) {
                    comp1.insert(e.eid, ci);
                } else {
    ...
    for i in 0..comps.len() {
        visited[i] = true;
        if comps[i].edges.is_empty() { continue; }

        let ti = ctype[i];
        if ti != SpqrNodeType::P && ti != SpqrNodeType::S { continue; }

        let mut ei = 0;
        while ei < comps[i].edges.len() {
            let eid = comps[i].edges[ei].eid;
            if (eid as usize) < m { ei += 1; continue; }
            ...
            for e in &j_edges {
                if (e.eid as usize) >= m {
                    if comp1.get(&e.eid) == Some(&j) { comp1.insert(e.eid, i); }
                    if comp2.get(&e.eid) == Some(&j) { comp2.insert(e.eid, i); }
                }
            }
...
fn assemble_spqr_tree(graph: &Graph, components: &[SplitComponent], next_virtual: u32) -> SpqrTree {
    ...
    for comp in components {
        ...
        let mut local_of = |v: u32| -> u32 {
            if let Some(&id) = lid.get(&v) { id }
            else { let id = n2o.len() as u32; lid.insert(v, id); n2o.push(NodeId(v)); id }
        };
        local_of(comp.pole_a); local_of(comp.pole_b);
        ...
        for edge in &comp.edges {
            ...
            if is_real { edge_to_tree_node[edge.eid as usize] = tid; }
            se.push(SkeletonEdge {
                src: ls, dst: ld,
                real_edge: if is_real { EdgeId(edge.eid) } else { EdgeId::INVALID },
        ...
        tree_nodes.push(SpqrTreeNode {
            node_type: nt,
            skeleton: Skeleton { num_nodes: n2o.len() as u32, edges: se, node_to_original: n2o },
            parent: TreeNodeId::INVALID, children: Vec::new(),
        });
    }
    ...
    for ti in 0..tree_nodes.len() {
        for ei in 0..tree_nodes[ti].skeleton.edges.len() {
            ...
            if vid == INVALID { continue; }
            assert!(vid >= base, "virtual_id {} < base {}", vid, base);
            let idx = (vid - base) as usize;
            assert!(idx < first.len(), "virtual_id {} out of range", vid);
            if let Some((tj, ej)) = first[idx].take() { pairs.push((ti, ei as u32, tj, ej)); }
            else { first[idx] = Some((ti, ei as u32)); }
        }
    }
    for entry in &first {
    ...
    for (a, ea, b, eb) in pairs {
        assert!(a != b);
        let ta = TreeNodeId(a as u32); let t_b = TreeNodeId(b as u32);
        if a < b {
        ...
        tree_adj[a].push(t_b); tree_adj[b].push(ta);
    }
    let root = if tree_nodes.is_empty() { TreeNodeId::INVALID } else { TreeNodeId(0) };
    if root.is_valid() {
        let mut par = vec![TreeNodeId::INVALID; tree_nodes.len()];
        let mut vis = vec![false; tree_nodes.len()];
        let mut st = vec![root]; vis[root.idx()] = true;
        while let Some(v) = st.pop() {
            for &u in &tree_adj[v.idx()] {
                if vis[u.idx()] { continue; }
                vis[u.idx()] = true; par[u.idx()] = v; st.push(u);
            }
        }
        if !vis.iter().all(|&b| b) {
        ...
        for i in 0..tree_nodes.len() { tree_nodes[i].parent = par[i]; tree_nodes[i].children.clear(); }
        for i in 0..tree_nodes.len() {
            let p = par[i];
            if p.is_valid() { tree_nodes[p.idx()].children.push(TreeNodeId(i as u32)); }
        }
    }
    SpqrTree { nodes: tree_nodes, root, edge_to_tree_node }
}
...
impl SpqrTree {
    pub fn normalize(&mut self) {
        ...
        while changed {
            ...
            for i in 0..self.nodes.len() {
                if self.nodes[i].skeleton.edges.is_empty() { continue; }
                let t = self.nodes[i].node_type;
                if t != SpqrNodeType::S && t != SpqrNodeType::P { continue; }
                let parent = TreeNodeId(i as u32);
                let children = self.nodes[i].children.clone();
                for child in children {
                    if !child.is_valid() || child.idx() >= self.nodes.len() { continue; }
                    if self.nodes[child.idx()].skeleton.edges.is_empty() { continue; }
    ...
    fn merge_into_parent(&mut self, parent: TreeNodeId, child: TreeNodeId) {
        let p_edge_idx = match self.nodes[parent.idx()].skeleton.edges.iter()
            .position(|e| e.virtual_id != INVALID && e.twin_tree_node == child) { Some(x) => x, None => return };
        let c_edge_idx = match self.nodes[child.idx()].skeleton.edges.iter()
            .position(|e| e.virtual_id != INVALID && e.twin_tree_node == parent) { Some(x) => x, None => return };
        ...
        for (i, &orig) in self.nodes[parent.idx()].skeleton.node_to_original.iter().enumerate() {
        ...
        let mut child_local_to_parent: Vec<u32> = Vec::with_capacity(child_n2o.len());
        {
            ...
            for &orig in &child_n2o {
                let pid = if let Some(&pid) = orig_to_parent.get(&orig.0) { pid }
                else { let pid = p_n2o.len() as u32; p_n2o.push(orig); orig_to_parent.insert(orig.0, pid); pid };
        ...
        for mut e in child_edges {
            ...
            if e.real_edge.is_valid() {
                ...
                if eid < self.edge_to_tree_node.len() { self.edge_to_tree_node[eid] = parent; }
            }
            let is_virtual = e.virtual_id != INVALID;
            let twin_tid = e.twin_tree_node; let twin_ei = e.twin_edge_idx;
            let new_idx = { let p = &mut self.nodes[parent.idx()].skeleton.edges; p.push(e); (p.len()-1) as u32 };
    ...
    pub fn compact(&mut self) {
        ...
        for (i, nd) in self.nodes.iter().enumerate() {
            if !nd.skeleton.edges.is_empty() { id_map[i] = TreeNodeId(new_n.len() as u32); new_n.push(nd.clone()); }
        }
        for nd in &mut new_n {
            if nd.parent.is_valid() { nd.parent = id_map[nd.parent.idx()]; }
            nd.children = nd.children.iter()
                .filter_map(|c| { let m = id_map[c.idx()]; if m.is_valid() { Some(m) } else { None } }).collect();
            for e in &mut nd.skeleton.edges {
                if e.twin_tree_node.is_valid() { e.twin_tree_node = id_map[e.twin_tree_node.idx()]; }
            }
        }
        for tn in &mut self.edge_to_tree_node { if tn.is_valid() { *tn = id_map[tn.idx()]; } }
        self.root = if self.root.is_valid() { id_map[self.root.idx()] } else { TreeNodeId::INVALID };
        self.nodes = new_n;
    }
}
...
impl fmt::Display for SpqrTree {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        for (i, nd) in self.nodes.iter().enumerate() {
            let t = match nd.node_type { SpqrNodeType::S=>"S", SpqrNodeType::P=>"P", SpqrNodeType::R=>"R" };
            writeln!(f, "  [{}] {}: {} edges, {} children, poles={:?}",
                i, t, nd.skeleton.num_edges(), nd.children.len(),
                if nd.skeleton.node_to_original.len()>=2 { nd.skeleton.poles() }
                else { (NodeId::INVALID, NodeId::INVALID) })?;
        }
        Ok(())
    }
}
...
mod tests {
    use super::*;
    fn make_k4() -> Graph {
        let mut g = Graph::with_capacity(4, 6); g.add_nodes(4);
        g.add_edge(NodeId(0),NodeId(1)); g.add_edge(NodeId(0),NodeId(2));
        g.add_edge(NodeId(0),NodeId(3)); g.add_edge(NodeId(1),NodeId(2));
        g.add_edge(NodeId(1),NodeId(3)); g.add_edge(NodeId(2),NodeId(3)); g
    }
    fn make_bond() -> Graph {
        let mut g = Graph::with_capacity(2, 3); g.add_nodes(2);
        g.add_edge(NodeId(0),NodeId(1)); g.add_edge(NodeId(0),NodeId(1));
        g.add_edge(NodeId(0),NodeId(1)); g
    }
    fn make_cycle(n: usize) -> Graph {
        let mut g = Graph::with_capacity(n, n); g.add_nodes(n);
        for i in 0..n { g.add_edge(NodeId(i as u32), NodeId(((i+1)%n) as u32)); } g
    }
    #[test] fn test_k4() {
        let t = build_spqr_tree(&make_k4());
        let (_s,_p,r) = t.count_by_type();
        assert!(r >= 1, "K4 needs R-node"); println!("{}", t);
    }
    #[test] fn test_bond() {
    ...
    #[test] fn test_cycle() {
        ...
        assert!(s >= 1, "Cycle needs S-node"); println!("{}", t);
    }
    #[test] fn test_single_edge() {
        let mut g = Graph::with_capacity(2,1); g.add_nodes(2);
        g.add_edge(NodeId(0),NodeId(1));
        assert_eq!(build_spqr_tree(&g).len(), 1);
    }
    #[test] fn test_edge_partition() {
        let g = make_k4(); let t = build_spqr_tree(&g);
        for i in 0..g.num_edges() {
            assert!(t.tree_node_of_edge(EdgeId(i as u32)).is_valid(), "Edge {} unmapped", i);
        }
    }
    #[test] fn test_triangle_single_s() {
        let g = make_cycle(3);
        let t = build_spqr_tree(&g);
        let report = verify::verify_spqr_tree_with_options(&g, &t, verify::VerifyOptions { require_reduced: false });
        assert!(report.is_ok(), "{:?}", report.errors);
        let (s, p, r) = t.count_by_type();
        assert_eq!((s, p, r), (1, 0, 0), "triangle should be one S-node");
    }
    #[test] fn test_two_triangles_shared_edge() {
        let mut g = Graph::with_capacity(4, 5); g.add_nodes(4);
        g.add_edge(NodeId(0), NodeId(1)); g.add_edge(NodeId(1), NodeId(2));
        g.add_edge(NodeId(2), NodeId(0)); g.add_edge(NodeId(0), NodeId(3));
    ...
    #[test] fn test_self_loops_separated() {
        let mut g = Graph::with_capacity(3, 5); g.add_nodes(3);
        g.add_edge(NodeId(0), NodeId(1));
        ...
        let report = verify::verify_spqr_tree_with_options(
            &g, &res.tree, verify::VerifyOptions { require_reduced: false },
        );
    ...
    #[test] fn test_only_self_loops() {
        let mut g = Graph::with_capacity(1, 3); g.add_nodes(1);
        g.add_edge(NodeId(0), NodeId(0));
        g.add_edge(NodeId(0), NodeId(0));
        g.add_edge(NodeId(0), NodeId(0));
        let res = build_spqr(&g);
    ...
    #[test] fn test_self_loops_with_multi_edges() {
        let mut g = Graph::with_capacity(2, 5); g.add_nodes(2);
    ...
    #[test] fn test_build_spqr_tree_debug_assert() {
        let g = make_k4();
        let t = build_spqr_tree(&g);
        assert!(!t.is_empty());
    }
