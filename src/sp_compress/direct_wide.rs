...
fn bucket_sort_core_items_by_pair(items: &mut [CoreItem]) {
    items.sort_unstable_by_key(|item| (item.u, item.v));
}
...
impl IncidentMap {
    fn new(n_nodes: usize, n_edges: usize) -> Self {
        ...
        if n_nodes <= MIN_DENSE_NODES || n_nodes <= dense_limit {
            return Self::Dense {
                inc0: vec![u64::MAX; n_nodes],
                inc1: vec![u64::MAX; n_nodes],
                touched: Vec::new(),
            };
        }
    ...
    #[inline]
    fn add(&mut self, v: u64, edge: u64) -> Option<()> {
        match self {
            Self::Dense {
                inc0,
                inc1,
    ...
    #[inline]
    fn other(&self, v: u64, edge: u64) -> Option<u64> {
        match self {
    ...
    fn all_touched_have_two_incidents(&self) -> bool {
        match self {
            Self::Dense {
                ...
                .iter()
                .all(|&v| inc0[v as usize] != u64::MAX && inc1[v as usize] != u64::MAX),
...
impl SparseIncidentMap {
    fn new(n_edges: usize) -> Self {
        ...
        Self {
            ...
            inc0: vec![u64::MAX; cap],
            inc1: vec![u64::MAX; cap],
            touched: Vec::new(),
            len: 0,
            mask: cap - 1,
        }
    ...
    fn grow(&mut self) {
        ...
        let old_keys = std::mem::replace(&mut self.keys, vec![u64::MAX; new_cap]);
        let old_inc0 = std::mem::replace(&mut self.inc0, vec![u64::MAX; new_cap]);
        let old_inc1 = std::mem::replace(&mut self.inc1, vec![u64::MAX; new_cap]);
        self.mask = new_cap - 1;
    ...
    #[inline]
    fn find_slot_for_insert(&self, key: u64) -> usize {
        let mut slot = Self::hash_u64(key) & self.mask;
        while self.keys[slot] != u64::MAX {
    ...
    fn add(&mut self, v: u64, edge: u64) -> Option<()> {
        ...
        if self.inc0[slot] == u64::MAX {
            self.inc0[slot] = edge;
            Some(())
        } else if self.inc1[slot] == u64::MAX {
            self.inc1[slot] = edge;
            Some(())
        } else {
            None
    ...
    fn all_touched_have_two_incidents(&self) -> bool {
        self.touched.iter().all(|&v| {
            ...
            self.inc0[slot] != u64::MAX && self.inc1[slot] != u64::MAX
        })
    }
}
...
trait DirectEdges {
    fn len(&self) -> usize;
    fn endpoint(&self, i: usize) -> (u64, u64);
...
pub(crate) fn try_compress_degree2_direct_indexed_u64(
    n_nodes: u64,
    src: &[u64],
    dst: &[u64],
    contractible: &[u8],
...
fn try_compress_degree2_direct_impl<E: DirectEdges>(
    ...
    let n = usize::try_from(n_nodes).ok()?;
    ...
    for i in 0..input_edges.len() {
        ...
        if u == v {
            ...
            if contractible[u] != 0 {
                incidents.add(u_raw, i as u64)?;
            }
            if contractible[v] != 0 {
                incidents.add(v_raw, i as u64)?;
            }
        }
    }
    ...
    for edge_idx in 0..input_edges.len() {
        ...
        let mut cur = start;
        let mut next_edge = edge_idx as u64;
        path_children.clear();
        loop {
    ...
    while i < core_items.len() {
        ...
        if u != v && j - i >= 2 {
            ...
            tree.stats.parallel_reductions += (j - i - 1) as u64;
        } else {
    ...
    for (word_idx, mut word) in core_node_bits.into_iter().enumerate() {
        while word != 0 {
            ...
            if node < n {
                tree.core_nodes.push(NodeId(node as u64));
            }
            word &= word - 1;
        }
    }
...
fn fbrq_contract_residual_degree2(
    ...
    for (eid, e) in edges.iter().enumerate() {
        ...
        if degree[e.u as usize] == 2 && incidents.add(e.u, eid as u64).is_none() {
            return edges;
        }
        if degree[e.v as usize] == 2 && incidents.add(e.v, eid as u64).is_none() {
            return edges;
        }
    }
    if !incidents.all_touched_have_two_incidents() {
    ...
    for start_eid in 0..edges.len() {
        ...
        if valid && path.len() >= 2 && degree[curr as usize] != 2 && curr != start {
            ...
            tree.stats.series_reductions = tree
                .stats
                .series_reductions
                .saturating_add((chain_children.len() - 1) as u64);
        } else {
            for &eid in &path {
...
#[inline]
fn other_contractible_edge_dense(v: usize, edge: u64, inc0: &[u64], inc1: &[u64]) -> Option<u64> {
    let a = inc0[v];
    let b = inc1[v];
    if a == edge && b != u64::MAX {
        Some(b)
...
fn make_path_child(
    ...
    tree.stats.series_reductions += (children.len() - 1) as u64;
